    ...
    <section ref={sectionRef} id="about" className="py-32 bg-background">
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <div className="mb-20 text-center">
          <h2
            className={`text-heading font-bold tracking-tight mb-4 transition-all duration-700 ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 md:grid-rows-2 gap-6 md:h-[800px]">
          ...
          <div
            className={`col-span-1 md:col-span-4 md:row-span-2 glass-card rounded-[40px] overflow-hidden relative group transition-all duration-700 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
            }`}
            style={{ filter: isVisible ? "blur(0)" : "blur(10px)" }}
          >
            <img
              src="https://images.unsplash.com/photo-1571266028243-371695039989?q=80&w=1200"
              alt="Live Set"
              className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 opacity-80 min-h-[400px]"
            />
          ...
          {/* Total Audio Card */}
          <div
            className={`col-span-1 md:col-span-2 md:row-span-1 glass-card rounded-[40px] p-8 flex flex-col justify-between transition-all duration-700 delay-100 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
            }`}
            style={{ filter: isVisible ? "blur(0)" : "blur(10px)" }}
          >
          ...
          <a
            href="#services"
            className={`col-span-1 md:col-span-2 md:row-span-1 glass-card rounded-[40px] p-8 relative overflow-hidden group cursor-pointer transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
            }`}
            style={{ filter: isVisible ? "blur(0)" : "blur(10px)" }}
          >
